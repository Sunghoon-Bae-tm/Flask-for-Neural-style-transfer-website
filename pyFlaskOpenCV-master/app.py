...
def upload():
	target = os.path.join(APP_ROOT, 'new_image/')
	print("TARGET", target)
...
def send_original_image(filename):
	return send_from_directory("new_image", filename)
...
def send_processed_image(filename, selected_style):
	directoryName = os.path.join(APP_ROOT, 'new_image/')

	newImg = neuralStyleProcess.neuralStyleTransfer(directoryName, filename, selected_style)
	
	return send_from_directory("new_image", newImg)
